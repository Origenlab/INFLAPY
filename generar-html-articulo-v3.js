// ============================================
// GENERADOR DE HTML ARTICULO - INFLAPY v3.0
// REPLICA EXACTA de renta-inflables-polanco-cdmx.html
// Layout 2 columnas + Sidebar completo + FAQ Accordion
// ============================================
...
const detalle1Alt = imgSEO?.detalle1?.altText || `Inflables para fiestas infantiles en ${t.zona} - diversion garantizada`;
const detalle2Alt = imgSEO?.detalle2?.altText || `Fiesta infantil con inflables en ${t.zona}`;
...
let seccionesHTML = '';
let tocItems = [];
let imagenDetalle1Insertada = false;
let imagenDetalle2Insertada = false;

if (c.secciones && c.secciones.length > 0) {
  c.secciones.forEach((sec, idx) => {
    // Crear ID limpio para la seccion
    ...
    if (sec.subsecciones && sec.subsecciones.length > 0) {
      sec.subsecciones.forEach((sub, subIdx) => {
        seccionesHTML += `
        <h3>${sub.h3}</h3>
        ${sub.contenido}`;
      });
    ...
    // Insertar imagen detalle 1 despues de seccion 2
    if (idx === 1 && !imagenDetalle1Insertada) {
      seccionesHTML += `
        <figure class="article-figure">
          <img src="${imgDetalle1}" alt="${detalle1Alt}" loading="lazy" width="800" height="500">
        </figure>`;
      imagenDetalle1Insertada = true;
    }

    // Highlight box de la seccion
    if (sec.highlightBox) {
      seccionesHTML += `
    ...
    // Insertar imagen detalle 2 despues de seccion 4
    if (idx === 3 && !imagenDetalle2Insertada) {
      seccionesHTML += `
        <figure class="article-figure">
          <img src="${imgDetalle2}" alt="${detalle2Alt}" loading="lazy" width="800" height="500">
        </figure>`;
      imagenDetalle2Insertada = true;
    }
  });
}
...
// ============================================
// GENERAR CHECKLIST (Servicio incluido)
// ============================================
const checklistHTML = `
        <ul class="checklist">
          <li><strong>Entrega a domicilio GRATIS</strong> en toda la zona de ${t.zona} y ${t.alcaldia}</li>
          <li><strong>Instalacion profesional</strong> por personal uniformado y capacitado</li>
          <li><strong>Motor-turbina industrial</strong> de alta potencia (silencioso)</li>
          <li><strong>Anclaje de seguridad</strong> adecuado para pasto o concreto</li>
          <li><strong>Tapete de proteccion</strong> en la entrada del inflable</li>
          <li><strong>Recogida puntual</strong> al finalizar tu evento</li>
          <li><strong>Inflables siempre limpios</strong> y desinfectados</li>
        </ul>`;
...
// ============================================
// HTML COMPLETO - REPLICA EXACTA DE POLANCO
// ============================================
